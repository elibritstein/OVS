...
AC_DEFUN([OVS_CHECK_DOCA], [
  AC_ARG_WITH([doca],
              [AS_HELP_STRING([--with-doca=static|shared],
                              [DOCA library link mode: shared ("=shared") or
                              static (default / "=static").  --with-dpdk is
                              required either way])],
              [have_doca=true])

  if test "$have_dpdk" != true || test "$with_dpdk" = no; then
    if test "$have_doca" = true; then
      AC_MSG_ERROR([Cannot compile with doca without dpdk, please add --with-dpdk])
    fi
  fi
