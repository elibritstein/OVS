...
{
    return CONTAINER_OF(netdev, struct netdev_dpdk, common.up);
}
...
netdev_dpdk_lookup_by_port_id(dpdk_port_t port_id, struct ovs_list *list)
{
    struct netdev_dpdk_common *common;
...
static struct netdev_dpdk *
netdev_netdev_dpdk_lookup_by_port_id(dpdk_port_t port_id)
    OVS_REQUIRES(dpdk_mutex)
{
    struct netdev_dpdk_common *common;

    common = netdev_dpdk_lookup_by_port_id(port_id, &dpdk_list);
...
netdev_dpdk_run(const struct netdev_class *netdev_class OVS_UNUSED)
{
    ...
    if (reset_seq != netdev_dpdk_last_reset_seq) {
        ...
        for (port_id = 0; port_id < RTE_MAX_ETHPORTS; port_id++) {
            ...
            ovs_mutex_lock(&dpdk_mutex);
            dev = netdev_netdev_dpdk_lookup_by_port_id(port_id);
            if (dev) {
                ovs_mutex_lock(&dev->common.mutex);
    ...
    if (new_devargs && new_devargs[0]) {
        ...
        if (!(dev->common.devargs && !strcmp(dev->common.devargs, new_devargs)
            ...
            if (!rte_eth_dev_is_valid_port(new_port_id)) {
                ...
                dup_dev = netdev_netdev_dpdk_lookup_by_port_id(new_port_id);
                if (dup_dev) {
