...
netdev_doca_get_esw_mgr_port_id(const struct netdev *netdev)
{
    ...
    int ret = NETDEV_DOCA_PORT_ID_INVALID;
...
netdev_doca_is_esw_mgr(const struct netdev *netdev)
{
    return netdev_doca_get_esw_mgr_port_id(netdev) ==
           ...
           netdev_doca_get_esw_mgr_port_id(netdev) != NETDEV_DOCA_PORT_ID_INVALID;
}
    ...
    OVS_REQUIRES(doca_mutex)
{
    ...
    dev->port_id = NETDEV_DOCA_PORT_ID_INVALID;
    dev->esw_mgr_port_id = NETDEV_DOCA_PORT_ID_INVALID;
    dev->attached = false;
...
netdev_doca_construct(struct netdev *netdev)
{
    ...
    ovs_mutex_lock(&doca_mutex);
    err = common_construct(netdev, NETDEV_DOCA_PORT_ID_INVALID, SOCKET0);
    ovs_mutex_unlock(&doca_mutex);
    ...
    OVS_REQUIRES(doca_mutex)
{
    ...
    if (!rte_eth_dev_is_valid_port(new_port_id)) {
        ...
        if (err) {
            new_port_id = NETDEV_DOCA_PORT_ID_INVALID;
        } else {
            new_port_id = netdev_doca_get_port_by_devargs(devargs);
            if (rte_eth_dev_is_valid_port(new_port_id)) {
                /* Attach successful */
    ...
    if (new_port_id == NETDEV_DOCA_PORT_ID_INVALID) {
