...
/* True when NAT IP attributes define an actual address range.
 * All-zero IP_MIN with no distinct max is direction-only, not an explicit
 * range. */
bool
conntrack_nat_ip_range_explicit(bool ip_min_specified, bool ip_max_specified,
                                size_t ip_addr_len,
                                const union ct_addr *min,
                                const union ct_addr *max)
{
    if (ip_addr_len == sizeof(ovs_be32)) {
        if (ip_min_specified && min->ipv4) {
            return true;
        }

        if (ip_max_specified && max->ipv4 != min->ipv4) {
            return true;
        }
    } else if (ip_addr_len == sizeof(struct in6_addr)) {
        if (ip_min_specified && !ipv6_mask_is_any(&min->ipv6)) {
            return true;
        }

        if (ip_max_specified && !ipv6_mask_is_any(&max->ipv6)
            && memcmp(&max->ipv6, &min->ipv6, sizeof max->ipv6) != 0) {
            return true;
        }
    } else if (ip_min_specified || ip_max_specified) {
        return true;
    }
...
void
conntrack_nat_set_explicit_range(struct nat_action_info_t *nat,
...
{
    nat->explicit_range =
        conntrack_nat_ip_range_explicit(ip_min_specified, ip_max_specified,
                                        ip_addr_len, &nat->min_addr,
                                        &nat->max_addr)
        || proto_min_specified || proto_max_specified;
}
    ...
    if (commit) {
        ...
        if (nat_action_info) {
            if (alg_exp) {
                ...
                nat_null_binding(ct, nc, nat_action_info);
            }
                 ...
                 const struct nat_action_info_t *nat_info)
{
     ...
     * connection. */
    if (!conn_key_lookup(ct, rev_key,
                         conn_key_hash(rev_key, ct->hash_basis),
                         time_msec(), NULL, NULL)) {
        return false;
    }
