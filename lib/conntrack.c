...
static void
nat_packet_fix_checksum(struct dp_packet *pkt)
{
    struct ip_header *nh = dp_packet_l3(pkt);
    size_t l4_size;

    if (!nh || dp_packet_l4_checksum_good(pkt)) {
        return;
    }
    ...
    if (nh->ip_proto == IPPROTO_TCP && l4_size >= TCP_HEADER_LEN) {
        ...
        uh->udp_csum = 0;
        uh->udp_csum = csum_finish(csum_continue(packet_csum_pseudoheader(nh),
                                                 uh, l4_size));
        if (!uh->udp_csum) {
    ...
    OVS_REQUIRES(ct->ct_lock)
{
    ...
    if (commit) {
        ...
        if (nat_action_info) {
            ...
            } else if (nat_has_explicit_range(nat_action_info, fwd_key_node->key.dl_type)
