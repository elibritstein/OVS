...
OVS must be configured with ``--with-dpdk`` and ``--with-doca``.  ``--with-doca`` is a
flag only; any argument (for example ``=static`` or ``=shared``) is ignored.
DOCA linking follows the ``--with-dpdk`` setting.
