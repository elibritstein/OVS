...
run_ovsdb_fsync_check() {
    ...
    if [ ! -x "${test_ovsdb}" ]; then
        echo "SKIP: ${test_ovsdb} not built yet"
        echo
        return
    fi

    echo "=== ovsdb log fsync checks (${test_ovsdb}) ==="
    stderr_file=$(mktemp)
    trap 'rm -f "$stderr_file"' RETURN

    if "${test_ovsdb}" -vconsole:warn log-fsync-check 2>"${stderr_file}"; then
        pass "test-ovsdb log-fsync-check"
    else
        fail "test-ovsdb log-fsync-check exited with an error"
    fi
