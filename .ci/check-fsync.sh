...
setup_runtime_library_path() {
    ...
    for dir in \
        "$(pwd)/dpdk-dir/lib/x86_64-linux-gnu" \
        "$(pwd)/ovsdb/.libs" \
        "$(pwd)/lib/.libs" \
        "$(pwd)/tests/.libs"; do
        if [ -d "${dir}" ]; then
...
resolve_test_ovsdb_binary() {
    ...
    for candidate in tests/.libs/test-ovsdb "${test_ovsdb}"; do
        ...
        if head -1 "${candidate}" 2>/dev/null | grep -q '^#!'; then
            continue
        fi
        echo "${candidate}"
        return 0
    ...
    if [ -x "${test_ovsdb}" ]; then
        echo "libtool:${test_ovsdb}"
        return 0
    fi
    return 1
}
...
run_test_ovsdb() {
    ...
    case "${binary}" in
    libtool:*)
        ./libtool --mode=execute "${binary#libtool:}" "$@"
        ;;
    *)
        "${binary}" "$@"
        ;;
...
EOF
    ...
    if ! run_cc -std=c11 -Wall -Wextra -o fsync-check fsync-check.c; then
...
run_ovsdb_fsync_check() {
    ...
    if run_test_ovsdb "${binary}" -vconsole:warn log-fsync-check 2>"${stderr_file}"; then
